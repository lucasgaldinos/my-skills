...
description: |
  Use this skill when generating a Python MCP (Model Context Protocol) server from scratch. Triggers on: "create an MCP server", "build a Python MCP server", "make a FastMCP server", "generate an MCP tool server in Python", "scaffold an MCP project", "I want to expose my API as an MCP server", "create MCP tools for Claude". Produces a complete, production-ready Python project using FastMCP with proper project structure (uv), type hints, docstrings, error handling, and transport configuration (stdio or streamable-http).
argument-hint: "[server name and list of tools/resources to expose]"
user-invocable: true
---
