::: {#jupyter-notebooks .section .level1}
# [](#jupyter-notebooks){.anchor aria-hidden="true"}Jupyter notebooks
...
Note that options and extensions that affect reading and writing of Markdown will also affect Markdown cells in ipynb notebooks. For example, [`--wrap=preserve`](#option--wrap){.option} will preserve soft line breaks in Markdown cells; [`--markdown-headings=setext`](#option--markdown-headings){.option} will cause Setext-style headings to be used; and [`--preserve-tabs`](#option--preserve-tabs%5B){.option} will prevent tabs from being turned to spaces.
:::
