::: {#custom-readers-and-writers .section .level1}
# [](#custom-readers-and-writers){.anchor aria-hidden="true"}Custom readers and writers
...
If the script is not found relative to the working directory, it will be sought in the `custom` subdirectory of the user data directory (see [`--data-dir`](#option--data-dir){.option}).
...
Note that custom writers have no default template. If you want to use [`--standalone`](#option--standalone){.option} with a custom writer, you will need to specify a template manually using [`--template`](#option--template){.option} or add a new default template with the name `default.NAME_OF_CUSTOM_WRITER.lua` to the `templates` subdirectory of your user data directory (see [Templates](#templates)).
:::
