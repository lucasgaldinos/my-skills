...
argument-hint: "Whenever: [you are designing or implementing a AI agent/command] [you need to create or follow XML tag standards for agent instructions, knowledge, examples, formatting, and orchestration]"
---
...
Task, bash, Read, TodoWrite, AskUserQuestion
</allowed_tools>
<forbidden_tools>
...
ALL design → architect agent</rule>
<rule scope="implementation">

ALL implementation → developer agent</rule>
<rule scope="review">

ALL reviews → reviewer agent</rule>
</delegation_rules>
...
Criteria for passing</status>
<status name="CONDITIONAL">

Criteria for conditional approval</status>
<status name="FAIL">

Criteria for failure</status>
</approval_criteria>
``````
...
5. **No Indentation** - Indentation inside tags is unnecessary; keep tags flush left
6. **Semantic Freedom** - Tags are semantic and can be created on the fly to describe any concept; you are not limited to predefined tags
7. **Line Jump After Opening Tag** - Opening tags must be followed by a blank line before content:
