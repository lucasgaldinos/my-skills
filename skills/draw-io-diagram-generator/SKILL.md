...
description: |
  Use this skill when creating, editing, or generating draw.io diagram files. Triggers on: "create a diagram", "draw a flowchart", "generate an architecture diagram", "design a sequence diagram", "make a UML class diagram", "build an ER diagram", "add a .drawio file", "update the diagram", "visualize the flow", "document the architecture", "show the data model", "diagram the service interactions". Covers mxGraph XML authoring, shape libraries, style strings, all major diagram types, the hediet.vscode-drawio VS Code extension, and the full workflow from request to a ready-to-open .drawio file.
