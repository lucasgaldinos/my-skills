...
name: obsidian-mcp
description: >
  Use when the Obsidian MCP server is active and the user wants to perform
  headless vault operations — read, write, search, or patch notes; manage tags,
  frontmatter, or daily notes; or run git-based vault backup and sync.
  Do not use for app-context actions (open in app, plugin debugging) — those use obsidian-cli.
metadata:
  version: "2.0"
  author: bitbonsai
---
